...
export const SessionLock: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [locked, setLocked] = useState(() => sessionStorage.getItem(LOCK_KEY) === 'true');
      ...
      <div className="min-h-screen flex items-center justify-center" style={{ background: 'var(--bg-primary)' }}>
        <div className="w-full max-w-xs text-center space-y-6 p-8">
          ...
          <div className="space-y-3">
            ...
            <button onClick={handleUnlock} className="btn-primary w-full">Unlock</button>
          </div>
        </div>
      </div>
    );
  ...
  if (setupMode) {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: 'var(--bg-primary)' }}>
        <div className="w-full max-w-xs text-center space-y-6 p-8">
          <Fingerprint size={32} style={{ color: '#818cf8' }} className="mx-auto" />
          <h2 className="text-lg font-bold text-foreground">Set Up PIN Lock</h2>
          <p className="text-sm text-muted">Choose a 4-6 digit PIN for session lock</p>
          <input
            type="password"
            inputMode="numeric"
            maxLength={6}
          ...
          <div className="flex gap-2">
            <button onClick={() => setSetupMode(false)} className="btn-secondary flex-1">Skip</button>
            <button onClick={handleSetPin} className="btn-primary flex-1">Set PIN</button>
          </div>
        </div>
      </div>
    );
