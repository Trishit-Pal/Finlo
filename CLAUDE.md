# CLAUDE.md — Finlo (Personal Expense Tracker)
...
- Login: Biometric (BiometricPrompt), 4 digit PIN, Google SSO
...
## Developer quick start (backend)

To run backend tests and development server locally:
...
Note: `requirements.txt` pins `bcrypt==3.2.0` to avoid a known incompatibility between `passlib[bcrypt]` and newer `bcrypt` releases; keep this pin when creating environments.
