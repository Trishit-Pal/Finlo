...
class OTPToken(Base):
    ...
    mobile_number_hash: Mapped[str] = mapped_column(String(128), index=True, nullable=False)
...
class User(Base):
    ...
    username_source: Mapped[Optional[str]] = mapped_column(String(16), nullable=True)  # manual/google/migration/admin
    full_name: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)
    avatar_url: Mapped[Optional[str]] = mapped_column(String(512), nullable=True)
    date_of_birth: Mapped[Optional[str]] = mapped_column(String(32), nullable=True)
    date_of_birth_source: Mapped[Optional[str]] = mapped_column(String(16), nullable=True)  # manual/google/migration/admin
    city: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)
    address: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)
    country: Mapped[Optional[str]] = mapped_column(String(128), nullable=True)
    currency: Mapped[str] = mapped_column(String(8), default="INR")
    mobile_number: Mapped[Optional[str]] = mapped_column(String(32), nullable=True)
    mobile_number_hash: Mapped[Optional[str]] = mapped_column(String(64), nullable=True, index=True)
    monthly_income: Mapped[Optional[str]] = mapped_column(Text, nullable=True)  # encrypted
    ...
    suggestions: Mapped[list["Suggestion"]] = relationship("Suggestion", back_populates="user", cascade="all, delete-orphan")
    feedbacks: Mapped[list["Feedback"]] = relationship("Feedback", back_populates="user", cascade="all, delete-orphan")
    budget_versions: Mapped[list["BudgetVersion"]] = relationship("BudgetVersion", back_populates="user", cascade="all, delete-orphan")
...
class Receipt(Base):
    ...
    duplicate_of_receipt_id: Mapped[Optional[str]] = mapped_column(String(36), ForeignKey("receipts.id", ondelete="SET NULL"), nullable=True)
    ...
    category_suggestion: Mapped[Optional[str]] = mapped_column(String(128), nullable=True)
...
class Transaction(Base):
    ...
    payment_mode: Mapped[Optional[str]] = mapped_column(String(32), nullable=True)  # cash/upi/card/net_banking
    tags: Mapped[Optional[list]] = mapped_column(JSON, default=list)
    is_recurring: Mapped[bool] = mapped_column(Boolean, default=False)
    recurrence_frequency: Mapped[Optional[str]] = mapped_column(String(32), nullable=True)  # daily/weekly/monthly/yearly
    source: Mapped[str] = mapped_column(String(32), default="manual")
    receipt_id: Mapped[Optional[str]] = mapped_column(String(36), ForeignKey("receipts.id", ondelete="SET NULL"), nullable=True)
    notes: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())

    user: Mapped["User"] = relationship("User", back_populates="transactions")
    receipt: Mapped[Optional["Receipt"]] = relationship("Receipt", back_populates="transactions")
...
class Bill(Base):
    ...
    frequency: Mapped[str] = mapped_column(String(32), default="monthly")  # once/weekly/monthly/quarterly/yearly
    category: Mapped[Optional[str]] = mapped_column(String(128), nullable=True)
    category_id: Mapped[Optional[str]] = mapped_column(String(36), ForeignKey("categories.id", ondelete="SET NULL"), nullable=True)
...
class Budget(Base):
    ...
    category: Mapped[str] = mapped_column(String(128), nullable=False)  # "overall" for total
    category_id: Mapped[Optional[str]] = mapped_column(String(36), ForeignKey("categories.id", ondelete="SET NULL"), nullable=True)
    limit_amount: Mapped[float] = mapped_column(Float, nullable=False)
    is_percentage: Mapped[bool] = mapped_column(Boolean, default=False)  # limit as % of income
...
class Debt(Base):
    ...
    user_id: Mapped[str] = mapped_column(String(36), ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True)
    name: Mapped[str] = mapped_column(String(255), nullable=False)
    type: Mapped[str] = mapped_column(String(64), nullable=False)  # personal_loan/credit_card/owed_to/owed_by
...
class BudgetVersion(Base):
    ...
    change_reason: Mapped[str] = mapped_column(String(32), nullable=False, default="update")
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
...
class UserConsent(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=gen_uuid)
    user_id: Mapped[str] = mapped_column(String(36), ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True)
    consent_type: Mapped[str] = mapped_column(String(64), nullable=False)  # statement_import/aggregator_link/email_parse/sms_parse
    scope: Mapped[str] = mapped_column(String(128), nullable=False, default="transactions")
    status: Mapped[str] = mapped_column(String(16), nullable=False, default="granted")  # granted/revoked
    ...
    __table_args__ = (
        Index("ix_user_consents_unique_scope", "user_id", "consent_type", "scope", unique=True),
    )
...
class Embedding(Base):
    ...
    __table_args__ = (
        Index("ix_embeddings_parent_type", "parent_id", "type"),
    )
