...
async def _build_coach_context(user: User, db: AsyncSession) -> dict:
        ...
        budget_status.append(
            {
                ...
                "spent": round(spent, 2),
                "percent": round(
                    (spent / b.limit_amount * 100) if b.limit_amount else 0, 1
                ),
            }
        )
    ...
    recurring_rows = await db.execute(
        select(
            ...
            func.count(func.distinct(func.substr(Transaction.date, 1, 7))).label(
                "month_count"
            ),
...
async def _call_coach_llm(context: dict) -> Optional[dict]:
    """Call LLM with the coach prompt. Falls back to mock on failure."""
    from app.services.llm_client import llm_chat_completion, llm_pool
    from app.services.prompts import COACH_PROMPT

    if not llm_pool():
        logger.warning("llm pool empty — returning mock coach output")
    ...
    try:
        response = await llm_chat_completion(
            ...
            max_tokens=800,
            temperature=0.3,
            op_name="coach.suggest",
        )
        return json.loads(response.choices[0].message.content or "{}")
    except Exception as e:
        logger.error("coach.llm_failed", extra={"err": repr(e)[:200]})
        return _mock_coach_output(context)
...
def _mock_coach_output(context: dict) -> dict:
    ...
    top_cat = (
        max(spend, key=spend.get, default="Dining & Restaurants")
        if spend
        else "Dining & Restaurants"
    )
    top_amount = spend.get(top_cat, 150.0)
    return {
        "summary": (
            f"Your highest spending this month is "
            f"{top_cat} at ${top_amount:.2f}. "
            f"Opportunities to reduce spending exist."
        ),
        "actions": [
            {
                "text": (
                    f"Set a weekly budget of "
                    f"${(top_amount * 0.75 / 4):.2f} "
                    f"for {top_cat}"
                ),
                "weekly_savings": round(top_amount * 0.25 / 4, 2),
                "rationale": (
                    f"You spent ${top_amount:.2f} on "
                    f"{top_cat} last month, 25% reduction "
                    f"is achievable."
                ),
                "source_receipts": [],
            }
...
async def generate_suggestions(
    user: User, receipt: Receipt, db: AsyncSession
) -> Optional[dict]:
