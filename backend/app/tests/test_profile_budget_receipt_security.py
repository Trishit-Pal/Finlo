...
async def test_statement_import_requires_explicit_consent(auth_client: AsyncClient):
    csv_payload = 'date,merchant,amount,category\n2026-04-01,Cafe Blue,250.00,Food & Dining\n'
...
async def test_receipt_duplicate_detection_and_confirm_idempotency(auth_client: AsyncClient):
    payload = {
        'text': 'ACME STORE\\nDate: 2026-04-01\\nTax 5.00\\nTOTAL 105.00',
        'confidence': 93,
    }
    ...
    linked = [t for t in txns.json()['items'] if t.get('receipt_id') == second_data['receipt_id']]
    assert len(linked) == 1
