...
async def test_signup_and_signin(client: AsyncClient):
    ...
    # Sign in
    resp2 = await client.post("/auth/signin", json={
        "email": "integration@example.com",
        "password": "SecurePass123!",
    })
...
async def test_budget_duplicate_rejected(auth_client: AsyncClient):
    payload = {"month": 2, "year": 2024, "category": "Dining & Restaurants", "limit_amount": 200.0}
...
async def test_submit_feedback(auth_client: AsyncClient):
    resp = await auth_client.post("/feedback", json={
        "rating": 4,
...
async def test_dashboard(auth_client: AsyncClient):
    ...
    await auth_client.post(
        "/transactions",
        json={"date": "2024-01-15", "merchant": "Starbucks", "amount": 8.50, "category": "Dining & Restaurants"},
    )
    await auth_client.post(
        "/transactions",
        json={"date": "2024-01-16", "merchant": "Whole Foods", "amount": 55.0, "category": "Groceries"},
    )
